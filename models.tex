...
Seamless integration for past and future values of time series is crucial for advanced applications. For example, in energy domain, it is important to balance the production and consumption of electricity by estimating the expected load within each sub-network, and interestingly, with renewable energy sources (e.g., solar and wind). The difference is supplied by conventional energy sources.   For this application, it is  more useful to provide  immediate approximate answers for queries than  later exact answers. Because  conventional sources (e.g., nuclear power) could not deliver energy instantaneously. %(other scenario where computing history is important for foreacasting)
...
TimeTravel compactly represents time series as models. As models are substantially smaller than the original time series, they  can be used to compute approximate queries one order of magnitude faster than when accessing the time series directly. In addition, model can be used as an access method over the underlying time series, this way we can solve we can access the relevant portions. These models are organized in a novel index denoted as \LNs. Extensive experimental analysis show  the speed up for approximate and exact queries  with varying the error guarantees.
...
Statistical  forecasting methods (e.g., ARIMA~\cite{tBOX76a}) use these components  to accurately compute forecasted values. For our purpose, we abstract a forecast method as a function of forecast parameters and states (i.e., past values) and outputs future values. We use this decomposition to represent the time series over the past and the future.
...
Figure~\ref{fig:example}a gives 50 values which is based on real-world household power consumption in UK.  This time series consists of three components (two seasonal  and one trend components)  as shown in Figure~\ref{fig:example}b. The seasonal components have periods of 10 and 5 cycles, respectively.
By storing the complements separately instead of the original timeseries,  we reduce the storage requirements. In the toy example, instead of storing the 50 values of the original time series, we need only  store 19 values to exactly represent the time series (i.e., with 0\% error and 100\% confidence)( four values for trend and 10 and five seasonality periods) which corresponds a compression ratio of 42\%. Moreover, we can forecast the time series by exaggerating the decomposed components.
\begin{figure*}[th]
\center
\subfigure{
